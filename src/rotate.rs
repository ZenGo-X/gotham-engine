...
pub trait Rotate {
    ...
    > {
        ...
        let rotate_commit_message = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotateCommitMessage1, RotateCommitMessage1);
        ...
        db_insert!(db, Some(claim.sub.clone()), Some(id.clone()), RotateRandom1, &random1);

        let (rotation_party_one_first, party_one_private_new) =
            party_one_master_key.rotation_first_message(&random1);

        db_insert!(db, Some(claim.sub.clone()), Some(id.clone()), RotateFirstMsg, &rotation_party_one_first);

        db_insert!(db, Some(claim.sub.clone()), Some(id.clone()), RotatePrivateNew, &party_one_private_new);
    ...
    async fn rotate_third(
        ...
        let rotate_party_one_private = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotatePrivateNew, Party1Private);
        ...
        db_insert!(db, Some(claim.sub.clone()), Some(id.clone()), RotateAlpha, &party_one_alpha);

        db_insert!(db, Some(claim.sub.clone()), Some(id.clone()), RotatePdlDecom, &party_one_pdl_decommit);
    ...
    async fn rotate_forth(
        ...
        let rotation_party_one_first = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotateFirstMsg, RotationParty1Message1);

        let rotate_party_one_private = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotatePrivateNew, Party1Private);

        let random = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotateRandom1, Rotation);

        // let tmp = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotateParty1Second);
        // let rotation_party_one_second = db_cast!(tmp, party_one::PDLSecondMessage);

        let rotation_party_two_first = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotateParty2First, Party2PDLFirstMessage);

        let party_one_alpha = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotateAlpha, Alpha);

        let party_one_pdl_decommit = db_get_required!(db, Some(claim.sub.clone()), Some(id.clone()), RotatePdlDecom, Party1PDLDecommit);
